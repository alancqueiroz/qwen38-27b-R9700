...
## AMD Radeon AI PRO R9700 (ROCm port)

The same stack builds and runs on AMD's RDNA4 workstation card (gfx1201,
32 GB): `docker compose -f docker-compose.amd.yml --profile single up -d`.
Same model prep, same patches, same speculative modes. No FlashInfer and no
Marlin there, so the KV-cache defaults shift (bf16 / int8-per-token-head
instead of fp8), batch mode runs W4A16 only, and the KV pool pins are scaled
to the larger card.

First real boot on that card (2026-08-26): server healthy in 210 s, KV pool
155,795 tokens (the 24 GB card fits ~73.8k), MTP acceptance identical to the
3090's (2.84-2.90 tok/step) at C1 = 65.7 tok/s decode and C8 = 257 aggregate —
speculation multiplies whatever the memory bus delivers, and this bus delivers
less; the honest per-config tables live in [docs/amd.md](docs/amd.md), along
with what ports, what substitutes, a RAM-lean requantizer for shared machines,
and a power-delivery caution raised by one hard shutdown under sustained bench
load.
